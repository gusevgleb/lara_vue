        ...
        <div id="integrations_state">
          ...
          >
            <template v-slot:item="{ item }">
              <tr :class="item.status === 'active' ? '' : 'table-danger'" :key="item.integrationId">
...
export default {
  name: "IntegrationListSuspended",

  created() {
    document.title = 'Список отключённых интеграций';
  },
  ...
  methods: {
    getIntegrations() {
      axios.get(this.apiUrl + 'internal/stata/drivers_disabled_modern', {})
        .then((response) => {
